...
| bash バージョン | **bash 4.x 以上を必須**とする | 連想配列を設定・状態の保持に用いる。3.2 互換のために区切り文字付き配列で代替すると、設定マージと状態機械の実装が著しく複雑になる |
...
| --- | --- | --- | --- |
| 1 | 2.1 依存コマンド | 「bash 3.2 でも動作するよう配慮する」 | **bash 4.0 以上を必須**とする。起動時にバージョンを判定し、満たさなければ終了コード 7 で終了する |
...
| SPECS.md の更新 | 1.2 節の 6 項目 | 実装着手前に SPECS.md へ反映する |
